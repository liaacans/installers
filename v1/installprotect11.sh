...
echo "🚀 Memasang proteksi Hide Node Information..."
...
class ServerViewController extends Controller
{
    ...
    public function show(Request $request, Server $server)
    {
        ...
        // Hilangkan informasi node untuk admin selain ID 1
        if ($user->id !== 1) {
            $server->setAttribute('node_id', null);
            $server->setAttribute('allocation_id', null);
        }
...
echo "✅ Proteksi Hide Node Information berhasil dipasang!"
echo "📂 Lokasi file: $REMOTE_PATH"
echo "🗂️ Backup file lama: $BACKUP_PATH (jika sebelumnya ada)"
echo "🔒 Informasi Node dan Tautan Daemon disembunyikan untuk admin selain ID 1"
